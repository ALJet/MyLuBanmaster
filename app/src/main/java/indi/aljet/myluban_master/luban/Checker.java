...
enum Checker {
    ...
    private boolean isJPG(byte[] data){
        ...
        byte[] signatureB = new byte[]{data[0],
        data[1],data[2]};
    ...
    /**
     * 这段代码不错 好像是android里面的源码
     * 旋转JPEG byte[]类型的图片(适合自定义相机拍出来回调的照片，PictureCallback里面)
     * @param jpeg
     * @return
     */
    private int getOrientation(byte[] jpeg){
        ...
        if(length > 8){
            ...
            while (count-- > 0 && length >= 12){
                ...
                if(tag == 0x0112){
                    int orientation = pack(jpeg,
                            offset + 8, 2,littleEndian);
                    switch (orientation){
                        case 1:
                            return 0;
                        case 3:
    ...
    String extSuffix(InputStreamProvider input){
        try{
            ...
            BitmapFactory.decodeStream(input.open()
            ,null,options);
    ...
    boolean needCompress(int leastCompressSize,
                         String path){
        if(leastCompressSize > 0){
            File source = new File(path);
            return source.exists() &&
                    source.length() >
                            (leastCompressSize << 10);
        }
        return true;
    }


    private int pack(byte[] bytes,int offset,
                     int length, boolean littleEndian){
        int step = 1;
        if(littleEndian ){
            offset += length -1;
            step = -1;
